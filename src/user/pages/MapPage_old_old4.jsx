...
// Prefer numeric PK; otherwise use a string code route
function detailPathFor(cp) {
  const pk = cp.pk ?? cp.id;
  const isNumericPk = pk !== undefined && pk !== null && String(pk).match(/^\d+$/);
  if (isNumericPk) return `/app/map/${pk}`;
  const code = cp.code ?? cp.cp_id ?? cp.slug ?? cp.identifier ?? null;
  if (code) return `/app/map/by-code/${encodeURIComponent(code)}`;
  return null;
}
  ...
  return (
    <div className="h-full w-full">
      ...
      <MapContainer className="h-full" center={[9.0108, 38.7613]} zoom={12} scrollWheelZoom>
        ...
        {points.map(cp => {
          const to = detailPathFor(cp);
          return (
            <Marker
              key={String(cp.pk ?? cp.id)}
            ...
            >
              <Popup>
                <div className="space-y-1">
                  <div className="font-semibold">{cp.name || cp.id || cp.pk}</div>
